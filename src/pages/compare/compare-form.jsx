...
function CompareForm() {
  ...
  useEffect(() => {
    if (
      data.length === 0 &&
      (dataMaster.countries.length === 0 ||
        (dataMaster.filteredCountries1.length === 0 &&
          dataMaster.filteredCountries2.length === 0))
    ) {
      ...
      if (getLocalstg) {
        ...
        axios
          .get(UrlConstant.FETCH_COUNTRIES)
          .then((response) => {
  ...
  useEffect(() => {
    setDataMaster((x) => ({
      ...x,
      filteredCountries2: selectedCountry1
        ? dataMaster.countries.filter(
            (country) => country.id !== selectedCountry1.id
          )
  ...
  useEffect(() => {
    setDataMaster((x) => ({
      ...x,
      filteredCountries1: selectedCountry2
        ? dataMaster.countries.filter(
            (country) => country.id !== selectedCountry2.id
          )
        : dataMaster.countries,
    }));
  }, [selectedCountry2]);
  ...
  const handleCompare = (e) => {
    ...
    if (selectedCountry1 && selectedCountry2) {
      ...
      getCountryForCompareByCCA3([param1, param2], dispatch)
        ...
        <form onSubmit={handleCompare}>
          <div className="row g-3">
            <CountrySelection
              ...
              data={dataMaster.filteredCountries1}
            />
            <CountrySelection
              country={selectedCountry2}
              onSelect={handleSelect2}
              onClear={handleClear2}
              data={dataMaster.filteredCountries2}
            />
          </div>
